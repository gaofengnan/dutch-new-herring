...
for (i in 1:length(vollaardvanours.df)) {
  # for (i in 1:10) {
  weight_match <- (as.numeric(vollaardvanours.df$Weight[i])*10 - as.vector(ours.df$weight))^2 <= 0.000001
  fat_match <- (as.numeric(vollaardvanours.df$Fat[i]) - as.vector(ours.df$fat_percentage))^2 <= 0.000001
  temp_match <- (as.numeric(vollaardvanours.df$Temperature[i]) - as.vector(ours.df$temp))^2 <= 0.000001
  final_match <- weight_match & fat_match & temp_match
